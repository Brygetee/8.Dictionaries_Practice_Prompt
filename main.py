...
# total = f"You have a total of {supplies['pins']} pins, {supplies['needles']} needles, and {supplies['cutting mat']} cutting mat."
...
#             #print statement printing all the supplies and their values
#             print(total)
#             break
#         else:
...
"""How to improve code
1. To make the total statement print cutting mats when there is more than one, a function is needed.
"""
...
def make_total():
    return (f"You have a total of {supplies['pins']} pins, "
            f"{supplies['needles']} needles, and "
            f"{supplies['cutting mat']} cutting "
            f"{'mats' if supplies['cutting mat'] > 1 else 'mat'}.")
...
while True:
    ...
    if result:
        print(f"You have {result} {status}.")
        while True:
            new_supplies_bought = input(f"Have you recently bought more supplies?\nIf so, which one? {supplies_list}, If none type 'none'.\n")
